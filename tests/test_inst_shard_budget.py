...
def test_the_m2_11_measured_projection_fits_with_recorded_headroom():
    ...
    # The refinement review's F3 added the v3 tombstone: 15,940 -> 15,941 and
    # headroom 2,060 -> 2,059, restated here together with the constant.
    assert (projected, GLOBAL_FILE_CAP - projected) == (15_941, 2_059), (
        f"headroom is {GLOBAL_FILE_CAP - projected:,}, not the 2,059 this suite"
...
def test_the_v1_transition_term_is_a_load_bearing_parameter():
    ...
    # M2-12 added the v2 tombstone beside v1 (Codex F3); the refinement review's
    # F3 added the v3 tombstone, so the term is 3. The property under test is that
    # the term is LOAD-BEARING — zeroing it must move the projection by exactly
    # its own size — not that it equals any one number.
    assert with_term - without == FILER_V1_TRANSITION_FILES == 3
