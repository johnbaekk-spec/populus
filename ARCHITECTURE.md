...
`code_sha` is the `populus` commit the site was built from; `dist_digest` is the digest the record job computes over the exact `dist/` tree that was uploaded (a **locally-computed** digest — Cloudflare exposes none, H1); `workflow_run_id` + `dist_artifact_id` + `dist_artifact_expires_at` locate the immutable workflow artifact holding those bytes and its retention deadline (CI holds it ≥ the supported rollback window, §5.5 retention); `cf_production_deployment_id` is the Cloudflare deployment made live — the rollback anchor. This document is **operational metadata about the dashboard, outside the data trust chain** (MCP clients never read it; the dashboard is not a pointer consumer, §5.5); it is nonetheless **independently attested** by the record job's workflow identity so the monitor and rollback (§13.2, §13.5) can trust the deployment IDs and digests they read. It is discovered by `build_id` convention, never referenced from the immutable manifest. Because the deploy job holds no GitHub-write scope and the record job holds no Cloudflare token, no single job can both deploy and forge its record (§14).
...
3. **Isolated deploy job.** Deployment is a separate job that runs **only after the publish job succeeds**. It holds **no GitHub write scopes** (no `contents: write`, no `id-token`, no `attestations`) and receives `CLOUDFLARE_API_TOKEN` as step-scoped env on the deploy steps only; the publish job never sees the token. `CLOUDFLARE_ACCOUNT_ID` is a non-secret Actions variable (§14). It first **recovers the immutable `dist/` workflow artifact** (published in step 2) into a read-only directory and **computes its digest locally** — this local digest, not any provider-side value, is the artifact identity used everywhere below (Cloudflare exposes no canonical content digest on a deployment, H1). Before any upload it **asserts production identity via the Cloudflare API** — reads the Pages project and checks that the workflow-locked branch name equals the project's configured `production_branch` (Pages project creation and that configuration are P3 setup items, §17); a mismatch aborts before uploading, because `--branch` selects production only when it matches the project's configured production branch.
4. **Preview upload, verify the preview first.** The job uploads the recovered `dist/` to a **non-production preview** deployment (`wrangler pages deploy dist/ --project-name <locked> --branch <preview alias>`), capturing the returned preview deployment ID and URL, then **live-verifies the preview URL** (cache-busted fetch; asserts the served page embeds this run's `build_id` + code SHA; asserts served `stats.json` hash equals the manifest-listed artifact). **Production is untouched** — a preview failure aborts here and alerts, production still serving the prior build.
5. **Production upload of the *same bytes*, verify, compensate on failure.** Only after the preview passes: **capture the current live production deployment ID** (the rollback anchor) → **upload the identical recovered `dist/` (same local digest — Cloudflare Direct Upload has no preview→production "promote" operation, so this is a second upload of provably the same bytes, not a provider-side promotion)** to the production branch → **live-verify the production custom domain** (same assertions). There is a brief window between the production upload and passing verification during which the domain serves the new build unverified-at-the-edge; the bytes are the same ones already verified on preview, and **any verification failure triggers an automatic Cloudflare rollback to the captured prior production deployment ID, re-verified** to confirm the domain serves the prior `build_id`. (Cloudflare rollback is an explicit API operation, not an automatic consequence of a failed external check — the job performs and verifies it.) A deployment is green only on live production proof, never on wrangler's exit status. **Honest characterization: two uploads of one locally-hash-pinned artifact, preview-verified first, with a compensating production rollback — not a transactional promotion** (the provider offers none).
6. **Record the deployment durably — separate document, separate job.** After production verifies, a dedicated **record job** (own `contents: write` + `attestations: write`, **no Cloudflare token** — §14) writes and attests `builds/<build_id>/deployment.json` (§5.5) from the deploy job's outputs: `code_sha`, the locally-computed `dist_digest`, `workflow_run_id` + `dist_artifact_id` + retention deadline, `cf_production_deployment_id`, `verified_at`. It is **not** written into the already-published immutable manifest (that would break its hash and attestation — C1); it is a separate, independently-attested, build-scoped document that makes rollback (§13.5) reproducible rather than "rebuild from current checkout and hope." The three-job split (publish / deploy / record) means no single job holds both the Cloudflare token and GitHub-write — so none can both deploy and forge the record.
...
- **Workflow least privilege — three jobs, disjoint privilege (§12.1).** Every workflow declares an explicit `permissions:` block; default `contents: read`. The **publish job** gets `contents: write` (only in `populus-data`) plus — from the P2 flip on — `id-token: write` + `attestations: write` (attestation scopes, §5.5); **no Cloudflare token**. The **deploy job** gets **only** `CLOUDFLARE_API_TOKEN` (step-scoped) and **no GitHub write scopes**. The **record job** (writes/attests `deployment.json`, §5.5) gets `contents: write` + `id-token: write` + `attestations: write` and **no Cloudflare token**. The invariant that matters: **no single job holds both the Cloudflare token and GitHub-write**, so none can both deploy the dashboard and forge or attest its deployment record.
...
**P3 — Dashboard (M1).** Scope: §9.10 + `/methodology`; Cloudflare Pages project created and its `production_branch` configured (§12.1 step 3); nightly rebuild via the §12.1 deployment protocol; localStorage follows. Gates: live on the domain; **per-deploy live verification green across ≥3 consecutive nightly deploys** (preview-then-production two-upload sequence, §12.1 steps 4–5: cache-busted fetch asserts embedded `build_id` + code SHA and that served `stats.json` hash equals the manifest artifact — every nightly proven, not just the first); **production-branch assertion fixture** — a deploy with a mismatched project `production_branch` aborts before upload; **deploy-failure fixtures** — (a) a preview that fails verification leaves production untouched, (b) a production-upload that fails post-upload verification triggers the compensating Cloudflare rollback to the captured prior deployment ID and re-verifies the restored `build_id`; `deployment.json` written + attestation-verifiable; `stats.json` byte-equality (canonical vs `dist/` vs live) holds; Lighthouse ≥90 (performance + accessibility) on feed, one member page, one ticker page; every rendered claim traceable to `doc_url` (spot-audit: 25 random rendered rows, 100% link-resolve); static file count within budget, **reported from the §12.1 pre-publication `dist/` count in `stats.json`**; **usage analytics live and verified privacy-clean** (§12.3 — a network trace of a page load shows no cookies and no cross-site identifiers, **no raw search text is transmitted or stored**, and OQ-14's consent-obligation determination for the chosen tool is on file); **dashboard-inclusive rollback drill** — the §13.5 sequence executed end to end from the target build's `deployment.json` with the domain live-verified on the rollback target, plus the §13.2 any-direction divergence alarm demonstrated against an artificially skewed deploy; second post published.
...
*End of ARCHITECTURE.md v2.9 — draft for owner review; supersedes all earlier versions (v2.1 = `f7985f6`, v2.2 = `5a665ce`, v2.3 = `cfdb972`, v2.4 = `1618fec`, v2.5 = `c85250b`, v2.6 = `a7dee2f`, v2.7 = `3784b25`, v2.8 = `b2ed2ea`). Finding dispositions for all review rounds: [REVIEW-RESPONSE.md](REVIEW-RESPONSE.md). No implementation begins until this document is approved (P0 gate).*
