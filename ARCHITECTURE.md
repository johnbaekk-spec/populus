...
Field semantics: `client_compat` is a **PEP 440 version-specifier string** evaluated against the client's own version (no `1.6.x`-style informal grammar); `normalization_version` and `digest_projection_version` key digest comparability (below); `logical_digest` is the canonical logical-content digest defined below; **every consumed file — SQLite, JSON slices, `stats.json` — is an enumerated artifact under a build-scoped path or Release URL.** The **`deployment` block is the durable dashboard record** (§12.1 step 6), one per build, its fields being: `code_sha` (the `populus` commit the site was built from), `dist_digest` (canonical digest of the deployed `dist/` tree), `workflow_run_id` + `dist_artifact_id` (the GitHub Actions run and the immutable upload-artifact holding `dist/`), `dist_artifact_expires_at` (that artifact's retention deadline), `cf_production_deployment_id` (the Cloudflare deployment made live — the rollback anchor), and `verified_at` (live production verification timestamp). It makes rollback (§13.5) reproducible — the target build's exact dashboard bytes are recoverable from `dist_artifact_id` (retained at least through `dist_artifact_expires_at`, which CI holds ≥ the supported rollback window, §5.5 retention) or, if the artifact has aged out, deterministically rebuilt from `code_sha` and cross-checked against `dist_digest`. `cf_production_deployment_id` lets the monitor and rollback address the exact Cloudflare deployment. The block is metadata *about* the dashboard, not a pointer consumer's trust input; it is not itself attested (the dashboard is outside the pointer trust chain, §5.5), but `dist_digest`/`code_sha` make any served divergence detectable (§13.2).
...
2. **Build before publish; count before freeze; stats into the bundle.** Stage the data build → build the site from the staged verified data → **enumerate `dist/` files (hard CI fail at the 15,000 cap; the count is a function of the file *list*, so it is final once the build exists) → write that count into the one `stats.json` in *both* places identically: the canonical staged artifact and `dist/stats.json`** → **assert the two `stats.json` copies are byte-equal** (writing a count into an existing file adds no files, so the enumeration stands) → assemble and hash the manifest over the canonical artifacts → `populus verify` → publish per §5.5 (assets → manifest → pointer). The published immutable `stats.json` and the `stats.json` inside the deployed bundle are therefore the same bytes; the per-deploy gate (step 5) re-checks this against the live site. The built `dist/` is handed forward as an immutable workflow artifact whose ID and digest are recorded in the deployment record (step 6).
3. **Isolated deploy job.** Deployment is a separate job that runs **only after the publish job succeeds**. It holds **no GitHub write scopes** (no `contents: write`, no `id-token`, no `attestations`) and receives `CLOUDFLARE_API_TOKEN` as step-scoped env on the deploy steps only; the publish job never sees the token. `CLOUDFLARE_ACCOUNT_ID` is a non-secret Actions variable (§14). Before any upload it **asserts production identity via the Cloudflare API** — it reads the Pages project and checks that the workflow-locked branch name equals the project's configured `production_branch` (Pages project creation and that configuration are P3 setup items, §17); a mismatch aborts before deploying, because `--branch` selects production only when it matches the project's configured production branch.
4. **Deploy to preview and verify there first (this is what makes it transactional).** The job uploads `dist/` to a **non-production preview** deployment (`wrangler pages deploy dist/ --project-name <locked> --branch <preview alias>`), capturing the returned preview deployment ID and URL, then **live-verifies the preview URL** (cache-busted fetch; asserts the served page embeds this run's `build_id` + code SHA; asserts served `stats.json` hash equals the manifest-listed artifact). **Production has not received the bytes yet** — a preview failure aborts here, alerts, and leaves production entirely untouched.
5. **Promote, then verify, then compensate on failure.** Only after the preview passes: **capture the current live production deployment ID** (the rollback anchor) → promote the *same verified `dist`* to production (deploy to the production branch; assert the promoted deployment's `dist` digest equals the digest just verified on preview — production only ever receives already-verified bytes) → **live-verify the production custom domain** (same assertions). **On any post-promote failure, automatically issue a Cloudflare rollback to the captured prior production deployment ID and re-verify that the domain now serves the prior `build_id`** — production self-heals to the last good deployment rather than being left on unverified bytes; the incident alerts regardless. (Cloudflare rollback is an explicit operation, not an automatic consequence of a failed external check — so the job performs and verifies it.) A deployment is green only on live production proof, never on wrangler's exit status.
6. **Record the deployment durably.** On success the job writes the build's **deployment record** into the manifest (§5.5): dashboard code SHA, canonical `dist` digest, the GitHub workflow run ID + `dist` artifact ID, the Cloudflare production deployment ID, and a retention deadline. This is what makes rollback (§13.5) reproducible rather than "rebuild from current checkout and hope."

If data publication fails after the site is built, the deploy job never runs — a site embedding an unpublished `build_id` cannot reach the domain. **Cloudflare Pages free-tier limits that do bind: 20,000 files, 25 MiB/file.** Global static-file cap: **15,000 files (75%)** — counting pages *and* deployed data shards — tracked per build in `stats.json` with a hard CI failure at the cap. Deploys: 1 nightly + manual. Per-module budgets are contract items (M1 ≤4,000 files; M2 ≤1,500 filer pages + aggregate slices; M3 ≤2,000 company pages + ~64 data shards). **Long-tail strategy (bounded):** entities within a module's *published extract* but beyond its pre-rendered page budget are served by a generic client-rendered route that fetches the **same-origin data shards deployed with the build** — no cross-origin fetches, no unversioned paths, coverage exactly equal to what §5.6 says is published. Entities outside the published extract link out to the primary source instead of rendering.
...
**Two aggregate channels, both $0:**

- **Web (dashboard).** Cookieless, privacy-first web analytics (candidate: **Cloudflare Web Analytics** — native to the Pages host, no cookies, no cross-site identifiers, and no consent banner required in typical jurisdictions because it stores no personal data; exact tool + free-tier terms confirmed at P3, OQ-14). Captures: page views and trend, **top member/ticker pages** (what draws attention), **referrers** (which launch post converted), coarse geography/device class, and a cookieless new-vs-returning estimate. The client-side search box is instrumented to log **only the query string** (a ticker or member name), truncated to a safe length and **never joined to an IP or any identifier** — "what people look for" without "who looked."
- **MCP (server).** The server runs on users' machines and **must not phone home** — silent telemetry from a local dev tool would violate the trust the project trades on (G15). Adoption is read only from aggregate signals the platforms already publish: **PyPI download counts** (installs + trend), **GitHub Release asset download counts** (snapshot pulls), registry listing views, stars/forks. "N installs this month" is the reputation metric, and it needs no telemetry.

**Not collected, at all, before P-Ω:** identity, cross-device linkage, per-user history, or any behavioral profile tied to a person. Those arrive only with **opt-in** accounts where the user trades identity for a feature (synced watchlist, alerts, API key) — never as a condition of reading public data, and even then login stays optional on the free tier.
...
**P3 — Dashboard (M1).** Scope: §9.10 + `/methodology`; Cloudflare Pages project created and its `production_branch` configured (§12.1 step 3); nightly rebuild via the §12.1 deployment protocol; localStorage follows. Gates: live on the domain; **per-deploy live verification green across ≥3 consecutive nightly deploys** (preview-then-production, §12.1 steps 4–5: cache-busted fetch asserts embedded `build_id` + code SHA and that served `stats.json` hash equals the manifest artifact — every nightly proven, not just the first); **production-branch assertion fixture** — a deploy with a mismatched project `production_branch` aborts before upload; **transactional-failure fixtures** — (a) a preview that fails verification leaves production untouched, (b) a post-promote verification failure triggers the compensating rollback to the captured prior deployment ID and re-verifies the restored `build_id`; `stats.json` byte-equality (canonical vs `dist/` vs live) holds; Lighthouse ≥90 (performance + accessibility) on feed, one member page, one ticker page; every rendered claim traceable to `doc_url` (spot-audit: 25 random rendered rows, 100% link-resolve); static file count within budget, **reported from the §12.1 pre-publication `dist/` count in `stats.json`**; **usage analytics live and verified privacy-clean** (§12.3 — a network trace of a page load shows no cookies and no identifiers, no consent banner is required, and the search-query log carries no IP/identifier join); **dashboard-inclusive rollback drill** — the §13.5 sequence executed end to end from the target build's deployment record with the domain live-verified on the rollback target, plus the §13.2 any-direction divergence alarm demonstrated against an artificially skewed deploy; second post published.
...
*End of ARCHITECTURE.md v2.8 — draft for owner review; supersedes all earlier versions (v2.1 = `f7985f6`, v2.2 = `5a665ce`, v2.3 = `cfdb972`, v2.4 = `1618fec`, v2.5 = `c85250b`, v2.6 = `a7dee2f`, v2.7 = `3784b25`). Finding dispositions for all review rounds: [REVIEW-RESPONSE.md](REVIEW-RESPONSE.md). No implementation begins until this document is approved (P0 gate).*
