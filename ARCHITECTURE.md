...
1. **Pinned inputs.** The site builds in the same workflow run as the data build, from the same `populus` checkout — the run records that code SHA, and the site embeds it in the footer beside `build_id`. Node toolchain pinned: a committed `.node-version`, a committed lockfile with a frozen install (`npm ci`), and Wrangler pinned to an exact *version-named* invocation (never `@latest`) — all under §14's SHA-pinning and Dependabot discipline. *(Accuracy note, RUN PUBLIC-SECURITY-HARDENING PR 2: today the deploy step runs `npx --yes wrangler@4.42.0` — version-exact but resolved at deploy time, **not** installed from the committed lockfile. Moving Wrangler into `dashboard/package-lock.json` and invoking the locally installed binary directly is that run's PR 4.)*
