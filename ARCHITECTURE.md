...
| `inst_serving.db` *(added 2026-08-02, M2-8)* | SQLite | Release assets | derived per-filer serving projection consumed by MCP; ~90 B/row target |
...
- **Path → URL.** URL = deployment origin + `/` + `path`, **verbatim, with no extension stripping and no directory-index rewriting** (an `index.html` entry is fetched at its literal `…/index.html` path). Requests are made with **redirects disabled**: any 3xx on an inventoried path is a verification failure (this is what makes an injected `_redirects` detectable on known paths).
...
10. **Deployment verification is expected-path, point-in-time, and not closure** (§5.5) — three declared limits, not one:
    ...
    - **`partial` is a real, gated mode.** If a tree exceeds the verification budget or fetches fail past retry, the signer attests `verification_scope: "partial"` (executable assets + entry pages + random sample) **honestly rather than overstating**, and alarms. At the CI-enforced page budgets (M1 ≤8,500; global 15,000) `expected_paths` is the expected path and `partial` should never fire in normal operation.
