...
**The record job is a *verifier*, not a signing oracle (C1).** It does not sign values handed to it by the privileged deploy job — an attestation would then only prove *who emitted* the values, not that they are *true*. Instead the record job **independently re-derives and re-checks every field before signing**: it takes `build_id`, `code_sha`, `workflow_run_id`, and `dist_artifact_id` from **GitHub Actions context and the publish job's signed outputs** (not from the deploy job); it **downloads the immutable `dist/` workflow artifact itself and recomputes `dist_digest`** (the canonical tree digest, §12.1 / `dist_digest_version`); and it accepts only the one value it cannot re-derive — the candidate `cf_production_deployment_id` — after **fetching both that deployment's Cloudflare deployment-specific URL and the live custom domain and asserting each serves the expected `build_id` + `code_sha` and a `stats.json` whose hash matches the manifest-listed artifact**. Any mismatch → it refuses to attest and alarms. So a compromised deploy job can still deface the dashboard (it holds the Cloudflare token — that residual is TD-8), but it **cannot make the record job attest a false record**: a lie about the digest or the deployment id fails the independent recompute or the live-URL check.

Field notes: `dist_digest` is the canonical tree digest defined in §12.1 (Cloudflare exposes none, H1); `workflow_run_id` + `dist_artifact_id` + `dist_artifact_expires_at` locate the immutable workflow artifact and its retention deadline (CI holds it ≥ the supported rollback window, §5.5 retention); `cf_production_deployment_id` is the live Cloudflare deployment — the rollback anchor. This document is **operational metadata about the dashboard, outside the data trust chain** (MCP clients never read it; the dashboard is not a pointer consumer, §5.5); it is **independently attested** by the record job's workflow identity so the monitor and rollback (§13.2, §13.5) can trust what they read. It is discovered by `build_id`/`generation` convention, never referenced from the immutable manifest.
...
6. **Record the deployment durably — separate document, separate job, independent verification.** After production verifies, a dedicated **record job** (own `contents: write` + `id-token: write` + `attestations: write`, **no Cloudflare token** — §14) writes and attests the next append-only generation `builds/<build_id>/deployments/<gen>.json` (§5.5). **It is a verifier, not a signing oracle (C1):** it takes `build_id`, `code_sha`, `workflow_run_id`, `dist_artifact_id` from **GitHub context + the publish job's signed outputs** (not the deploy job); **independently downloads the immutable artifact and recomputes `dist_digest`**; and accepts the deploy job's candidate `cf_production_deployment_id` **only after fetching that deployment's Cloudflare URL and the live custom domain and confirming both serve the expected `build_id` + `code_sha` + manifest-matching `stats.json`**. Mismatch → refuse to attest + alarm. The record is **not** written into the immutable manifest (that would break its hash and attestation — C1). Result: a compromised deploy job can deface the dashboard (it holds the token — TD-8) but **cannot cause a false record to be attested**, because the record job re-derives or live-verifies every field rather than trusting deploy-job outputs. If the record job cannot produce a verified record, it **fails closed** and the next publish is gated (§13.2, §17) until a valid attested generation exists for the live build.
...
- **Workflow least privilege — three jobs, disjoint privilege (§12.1).** Every workflow declares an explicit `permissions:` block; default `contents: read`. The **publish job** gets `contents: write` (only in `populus-data`) plus — from the P2 flip on — `id-token: write` + `attestations: write` (attestation scopes, §5.5); **no Cloudflare token**. The **deploy job** gets **only** `CLOUDFLARE_API_TOKEN` (step-scoped) and **no GitHub write scopes**. The **record job** (writes/attests the deployment generation, §5.5) gets `contents: write` + `id-token: write` + `attestations: write` and **no Cloudflare token**. Two invariants, stated precisely (C1): (a) **no single job holds both the Cloudflare token and GitHub-write**; (b) more importantly, the record job **independently re-derives or live-verifies every field it attests** rather than signing deploy-job outputs — so a compromised deploy job can deface the live dashboard (it holds the token; residual TD-8) but **cannot cause a false deployment record to be attested**. The attestation means "the record job checked this," not merely "some job asserted this."
...
*End of ARCHITECTURE.md v2.10 — draft for owner review; supersedes all earlier versions (v2.1 = `f7985f6`, v2.2 = `5a665ce`, v2.3 = `cfdb972`, v2.4 = `1618fec`, v2.5 = `c85250b`, v2.6 = `a7dee2f`, v2.7 = `3784b25`, v2.8 = `b2ed2ea`, v2.9 = `0f44bec`). Finding dispositions for all review rounds: [REVIEW-RESPONSE.md](REVIEW-RESPONSE.md). No implementation begins until this document is approved (P0 gate).*
