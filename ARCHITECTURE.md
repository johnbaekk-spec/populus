...
Astro static on Cloudflare Pages, **deployed publisher-side** (no `latest.json` resolution — the dashboard is not a pointer consumer and holds no replay state, which is what closes the ephemeral-build TOFU hole) via `wrangler pages deploy` (direct upload, free tier). The Pages-side build system is unused, so its **20-minute build timeout does not apply**; the free tier's **500 monthly deployments remain a tracked ceiling** (§13.4) — Cloudflare's limits page words it against Git builds and the Direct Upload guide does not explicitly exempt uploads, so at ~35 deploys/month the >14× headroom is tracked, not assumed away. No backend; localStorage personalization; no browser calls to external APIs (§5.6).
...
2. **Build before publish; count before freeze.** Stage the data build → build the site from the staged verified data → **count `dist/` files (hard CI fail at the 15,000 cap) and write the actual count into `stats.json` before the manifest hashes it** — the published, immutable `stats.json` reports the real deployed file count, not an estimate — → assemble the manifest → `populus verify` → publish per §5.5 (assets → manifest → pointer). The built `dist/` is handed forward as an immutable workflow artifact.
3. **Isolated deploy job.** Deployment is a separate job that runs **only after the publish job succeeds**. It holds **no GitHub write scopes** (no `contents: write`, no `id-token`, no `attestations`) and receives `CLOUDFLARE_API_TOKEN` as step-scoped env on the single deploy step only: `wrangler pages deploy dist/ --project-name <locked in the workflow file> --branch <production branch>` — the explicit production branch makes the upload a production deployment, never a preview. Conversely, the publish job never sees the Cloudflare token. `CLOUDFLARE_ACCOUNT_ID` is a non-secret Actions variable (§14).
4. **Live verification, every deploy.** The deploy job then fetches the custom domain with a cache-busting query and asserts the served page embeds **this run's `build_id` and code SHA** — a deployment is green only on live-domain proof, never on wrangler's exit status. Failure alerts and leaves the previous deployment serving (the data build is already published and correct; the site skew is then the §13.2 divergence alarm's business until redeploy).
...
| Pages deployments/month | >400 (of the free tier's 500) | reduce manual deploys; confirm direct-upload metering with Cloudflare before relying on more |
...
**P3 — Dashboard (M1).** Scope: §9.10 + `/methodology`; nightly rebuild via the §12.1 deployment protocol; localStorage follows. Gates: live on the domain; **per-deploy live verification green across ≥3 consecutive nightly deploys** (the §12.1 step: cache-busted fetch asserts the embedded `build_id` + code SHA — every nightly is proven, not just the first); Lighthouse ≥90 (performance + accessibility) on feed, one member page, one ticker page; every rendered claim traceable to `doc_url` (spot-audit fixture: 25 random rendered rows, 100% link-resolve); static file count within budget, **reported from the §12.1 pre-publication `dist/` count in `stats.json`**; **dashboard-inclusive rollback drill** — the §13.5 sequence executed end to end with the domain live-verified on the rollback target, plus the §13.2 any-direction divergence alarm demonstrated against an artificially skewed deploy; second post published.
...
8. **The deploy job runs a second toolchain against a production credential** (§12.1): Node/Astro/Wrangler execute with the Cloudflare token in scope. Bounded by: the committed lockfile + frozen install, the exact Wrangler pin, SHA-pinned actions, the job split (no GitHub write scopes co-resident with the token), the dedicated Cloudflare account (§14), and per-deploy live verification. The residual — an npm-ecosystem compromise inside the deploy job — is accepted and reviewed with §14's quarterly secret review; its worst case is dashboard defacement, never data-artifact tampering (artifacts stay attestation-verified end to end).
