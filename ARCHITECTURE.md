...
10. **Deployment verification is expected-path, point-in-time, and not closure** (§5.5) — three declared limits, not one:
    ...
    - **`partial` is a real, gated mode.** If a tree exceeds the verification budget or fetches fail past retry, the signer attests `verification_scope: "partial"` (executable assets + entry pages + random sample) **honestly rather than overstating**, and alarms. At the CI-enforced page budgets (M1 ≤4,000; global 15,000) `expected_paths` is the expected path and `partial` should never fire in normal operation.
