...
`pointer_version` is a strictly monotonic integer across all generations; `expires_at` bounds staleness (7 days — refreshed by every nightly publish). From P2 on, **each pointer generation is itself attested** (its exact bytes are an attestation subject like any artifact). Clients enforce: attestation valid → `pointer_version` **strictly greater than the highest version ever accepted** (persisted in the local cache) → not expired → `manifest_sha256` matches the fetched manifest. A compromised contents-write credential can rewrite the file but cannot mint a valid attestation for it, and re-serving an older *legitimately attested* pointer fails the monotonicity check. **Legitimate rollback is a new, higher `pointer_version` targeting an older `build_id`** (runbook §13.5) — cryptographically distinguishable from replay. Unversioned convenience copies may exist for humans but are non-normative and never consumed programmatically; everything else a consumer touches is under `builds/<build_id>/…` or an immutable release tag, so builds cannot mix.
...
**Consumer protocol (MCP server and site build):** fetch `latest.json` → **verify the pointer** (attestation by its byte digest; `pointer_version` strictly above the persisted highest-accepted; not expired) → fetch the manifest at `manifest_path`, check it hashes to `manifest_sha256`, **verify its attestation** → evaluate `client_compat` against own version — **on incompatibility, refuse with a self-explanatory message and continue serving the last compatible cached build** → download artifacts to temp files → verify SHA-256 + byte size → for SQLite, `PRAGMA integrity_check` → atomic rename into `~/.cache/populus/<module>/<build_id>/` → persist the accepted `pointer_version` → update a local per-build `current` pointer. Any failure at any step leaves the prior cache and highest-accepted state untouched; artifacts from different builds are never mixed.
...
- **External heartbeat (independent of GitHub — review F12), protocol-conformant:** a launchd job on the Mac mini every 6 h behaves like any other consumer (§5.5): resolve `latest.json` → verify the pointer (attestation + monotonicity + expiry from P2 on; authenticated-API inside the private staging boundary during P1) → authenticate the manifest → fetch the **manifest-listed** `stats.json` artifact → verify its SHA-256 → then evaluate: build age >36 h, freshness lagging the manifest watermarks, or two consecutive fetch/verification failures → Discord alert. It never reads a mutable root path other than `latest.json`, so pointer state and statistics can never mix across builds. It additionally re-checks that the **immutable-releases setting is still enabled** on `populus-data` — via the repository-settings API, which requires the PAT's **Administration: read** scope (inventoried in §14) — and alerts if it ever flips off. This catches disabled schedules, dropped cron events, GitHub outages, and setting drift — the failure classes an Actions-hosted watchdog shares with the thing it watches. Operational-by-P1 is a gate.
...
- **Secrets inventory (exactly three, reviewed quarterly):** Discord webhook URL (alerting); Cloudflare Pages deploy-hook URL (rotated on any suspicion; invocations visible in Pages logs); one fine-grained PAT for the Mac mini — `populus-data`: **Contents read/write** (fallback publishing; read needed while the repo is private staging) **plus Administration: read, permanently** — the immutable-releases settings endpoint requires it, and the monitor's setting-drift check (§13.2) runs forever, not just during staging. Stored in macOS Keychain on the mini, never in dotfiles; Contents-read necessity lapses at the P2 flip, Administration-read does not.
...
- Attestation drill: the shipped client verifies a genuine pointer + manifest attestation (pinned identity + issuer) and **rejects** (a) a tampered artifact, (b) a manifest attested by a different workflow identity, and (c) a **replayed older attested pointer** (monotonicity fixture) (fixture tests, CI-retained).
...
7. **First-run pointer trust is TOFU** (§5.5): a brand-new client has no persisted highest-accepted `pointer_version`, so on its very first run a replayed older-but-attested, unexpired pointer would be accepted. Bounded by pointer expiry (≤7 days of staleness) and eliminated from the second run on. Accepted; shipping a pinned floor version in each client release would shrink the window further and is noted as a P2 implementation option.
...
*End of ARCHITECTURE.md v2.4 — draft for owner review; supersedes all earlier versions (v2.1 = commit `f7985f6`, v2.2 = `5a665ce`, v2.3 = `cfdb972`). Finding dispositions for all review rounds: [REVIEW-RESPONSE.md](REVIEW-RESPONSE.md). No implementation begins until this document is approved (P0 gate).*
