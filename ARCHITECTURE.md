...
**Pointer state machine.** Every verifier persists the last accepted `(pointer_version, pointer_sha256)` tuple and evaluates each fetched pointer four ways: **lower version** → reject as replay · **equal version, identical bytes** → **idempotent accept** — the normal case for every poll between publishes; no state change, no alarm · **equal version, different bytes** → reject and **alarm as equivocation** (two distinct pointers at one version can only mean publisher error or compromise) · **higher version** → verify attestation + expiry + `manifest_sha256`, install, then **atomically persist the new tuple**. A first-ever run has no tuple (TD-7, MCP clients only — see consumer set below). A compromised contents-write credential can rewrite the file but cannot mint a valid attestation for it; re-serving an older legitimately attested pointer fails the lower-version check. **Legitimate rollback is a new, higher, attested `pointer_version` targeting an older `build_id`** (exact sequence: runbook §13.5) — cryptographically distinguishable from replay.
...
Astro static on Cloudflare Pages, **deployed publisher-side**: the nightly publish workflow, immediately after `populus verify` passes on the build it just published, builds the site **from that exact verified build** (no `latest.json` resolution — the dashboard is not a pointer consumer and holds no replay state, which is what closes the ephemeral-build TOFU hole) and deploys with `wrangler pages deploy` (direct upload, free tier; the Pages-side build system is unused, so the 500-builds/month and 20-minute-build limits never bind). No backend; localStorage personalization; no browser calls to external APIs (§5.6). **Cloudflare Pages free-tier limits that do bind: 20,000 files, 25 MiB/file.** Global static-file cap: **15,000 files (75%)** — counting pages *and* deployed data shards — tracked per build in `stats.json` with a hard CI failure at the cap. Deploys: 1 nightly + manual. Per-module budgets are contract items (M1 ≤4,000 files; M2 ≤1,500 filer pages + aggregate slices; M3 ≤2,000 company pages + ~64 data shards). **Long-tail strategy (bounded):** entities within a module's *published extract* but beyond its pre-rendered page budget are served by a generic client-rendered route that fetches the **same-origin data shards deployed with the build** — no cross-origin fetches, no unversioned paths, coverage exactly equal to what §5.6 says is published. Entities outside the published extract link out to the primary source instead of rendering.
...
- **Secrets inventory (exactly three, reviewed quarterly):** Discord webhook URL (alerting); a Cloudflare API token scoped to Pages-edit on the single dashboard project (used by the publish workflow's `wrangler pages deploy`, §12.1 — replaces the former deploy-hook URL; rotated on any suspicion); one fine-grained PAT for the Mac mini — `populus-data`: **Contents read/write** (fallback publishing; read needed while the repo is private staging) **plus Administration: read, permanently** — the immutable-releases settings endpoint requires it, and the monitor's setting-drift check (§13.2) runs forever, not just during staging. Stored in macOS Keychain on the mini, never in dotfiles; Contents-read necessity lapses at the P2 flip, Administration-read does not.
...
7. **First-run pointer trust is TOFU — MCP clients only** (§5.5): a brand-new client has no persisted `(pointer_version, pointer_sha256)` tuple, so on its very first run a replayed older-but-attested, unexpired pointer would be accepted once; the persisted tuple closes the window from then on. Bounded by pointer expiry (≤7 days of staleness). **The other consumers do not share this window:** the external monitor's tuple lives on the mini's durable disk, and the dashboard is not a pointer consumer at all — it is deployed publisher-side with the just-verified build (§12.1), which is precisely why ephemeral site builds cannot re-enter TOFU. Shipping a pinned floor version in each client release would shrink the MCP window further and is noted as a P2 implementation option.
...
*End of ARCHITECTURE.md v2.5 — draft for owner review; supersedes all earlier versions (v2.1 = commit `f7985f6`, v2.2 = `5a665ce`, v2.3 = `cfdb972`, v2.4 = `1618fec`). Finding dispositions for all review rounds: [REVIEW-RESPONSE.md](REVIEW-RESPONSE.md). No implementation begins until this document is approved (P0 gate).*
