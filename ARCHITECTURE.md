...
| | |
|---|---|
| Status | **As-built specification of the deployed system.** publicfilings.org is live and serving attested builds; M1 (congressional) and M2 (institutional 13F) are shipped, the §12.1 deploy protocol and §5.5 publication protocol are the operating reality, and this document is maintained to match the system as it exists. The revision-history rows below are the durable audit trail of how it got here and are kept verbatim by design (several are pinned by tests). Open work lives in `docs/roadmap.md`. |
...
| Signer-dispatch incident amendment | **The environment boundary is a separately dispatched top-level signer, not a reusable called job** (2026-09-01). Publish run `33421701055` deployed verified bytes but its reusable signer received empty `DATA_REPO_PAT`/`CLOUDFLARE_PAGES_READ_TOKEN` values and failed before a verification verdict. Isolated same-run probe `33473559423` bound a direct job and a reusable called job to one temporary environment/secret: the direct job reported `empty=false`, length 27; the called job reported `empty=true`, length 0. The probe branch, PR, environment, and secret were removed. `publish.yml:sign` now holds no environment or production secret and only `actions: write`; it dispatches `record-sign.yml@main` with a unique request id and watches that exact run. The top-level signer owns `production-record-sign`, validates the source run's workflow path/branch/event/status through GitHub's API, cross-checks its head SHA against the artifact, downloads the cross-run artifact with `actions: read`, and uses only `contents: read` + OIDC/attestation write for provenance while retaining the existing `record-sign.yml@refs/heads/main` attestation identity. An unarmed signer fails in its first step instead of skipping green. This also creates a fully verifying retained-artifact recovery path for an operational pre-verdict failure; it is not an attestation bypass and cannot clear a content rejection. Repository-scope secret copies remain an owner-controlled, workflow-unreferenced fallback until one supervised full run succeeds; their deletion and the no-fallback second run remain owner-only. Historical revision rows below remain verbatim. | §5.5, §12.1, §14, §17 |
...
| Companions | `HANDOFF-REVIEW.md` (review of the original handoff) and `REVIEW-RESPONSE.md` (finding dispositions for all review rounds) are completed review records; both live in Git history. |
...
**Signer identity, stated precisely (round-10 H1; amended by the 2026-09-01 incident).** GitHub attestations identify a **workflow**, not an individual job within one, so "the record job's identity" is not a verifiable thing. Record verification and signing therefore live in their own **top-level workflow** (`.github/workflows/record-sign.yml`), dispatched and watched by the publishing workflow; its `job_workflow_ref` remains the distinct, pinnable identity. Verifiers pin **`…/record-sign.yml@refs/heads/main`** for deployment generations and **`…/publish.yml@refs/heads/main`** for the manifest and pointer (§5.5 client verification), both with issuer `https://token.actions.githubusercontent.com`. Because the deploy job holds Pages Write while the publish-side dispatcher holds only Actions write and signing runs in a separate top-level workflow that holds only Pages Read, compromise of the deploy job cannot produce a record bearing the record-signer identity. The dispatch conversion changes orchestration, not this certificate identity.
...
- *Private staging phase (P1) — ~~unattested by necessity~~, ACL-bounded by design.* **Corrected by RUN P3-3a (2026-08-03)** — the premise below assumed the attesting workflow lived in the private `populus-data`. It lives in `populus`, which is **public**, so artifact attestations are available now and P1 **is** attested. The property (nothing is trusted unsigned) is unchanged; only the mechanism moved. Decision record: `docs/operations/attestation.md`.  **GitHub artifact attestations are unavailable to private repositories on the Free plan** (Enterprise-only), so P1 builds carry none. None are needed: the only consumers are the pipeline itself and the external monitor, both reading through the **authenticated GitHub API inside the private repo's access boundary**, verifying manifest-listed SHA-256s and the (unattested) pointer. The trust boundary *is* the repo ACL — stated as such, not disguised as cryptography.
...
### 9.6 Backfill import and kadoa lineage
...
6. **Record the deployment durably — separate document, separate top-level *workflow*, inventory-wide verification.** After production verifies, an unprivileged hosted job in `publish.yml` dispatches **`record-sign.yml@main`** with a unique request id and watches that exact run. The dispatcher holds only `actions: write`, no environment, no production secret, and no attestation authority. The signer holds `actions: read` + `contents: read` + `id-token: write` + `attestations: write`, plus its **own `Pages Read`-only Cloudflare token — never `Pages Write`** (§14), and writes/attests the next append-only generation `builds/<build_id>/deployments/<gen>.json` (§5.5). The source run id is only a locator: the signer validates through GitHub's API that it names a `main`-branch `publish.yml` run with an allowed event/status, cross-checks the API head SHA against the downloaded artifact's `code_sha`, and records the source publish run id rather than its own orchestration run id. It trusts **no deploy-job output**: `build_id` comes from the attested published manifest/pointer; `code_sha`, `dist_digest`, and the inventory come from the immutable cross-run artifact it downloads itself (recomputing `inventory_digest` before trusting an entry); the production deployment id is read from the **Cloudflare Pages API with its read token** and cross-checked against the deploy job's claim. It then **fetches every inventory path from that deployment's URL (redirects disabled) and verifies each decoded body hash and length**, runs the three closure-narrowing provider checks (no Functions, control-path 404 probes, header allowlist — §5.5), confirms the custom domain serves the same deployment, and attests with `verification_scope: "expected_paths"` recording exactly what it checked. Any mismatch → refuse to attest + alarm. A separate workflow is what makes the signer identity distinct and pinnable (attestations identify workflows, not jobs — §5.5); making it top-level is what lets its branch-restricted environment resolve. If no verified record can be produced, the watched run **fails closed** and the next publish is gated (§13.2, §17) until a valid attested generation exists for the live build.
...
### 13.5 Runbooks (summarised here; most operator procedures are not
published — rollback, disaster recovery and the self-hosted runner remain
in-tree because gates assert through them)
...
- **Secret residence — three branch-restricted GitHub environments (RUN PUBLIC-SECURITY-HARDENING R4/LD5; signer topology amended 2026-09-01).** The production workflow path resolves its three GitHub Actions secrets (`DATA_REPO_PAT`, `CLOUDFLARE_PAGES_EDIT_TOKEN`, `CLOUDFLARE_PAGES_READ_TOKEN`) from environments: `production-data-publish` (publish job — data PAT only), `production-pages-deploy` (deploy job — Pages Write only, `contents: read`), and `production-record-sign` (the top-level signer's `record` job — data PAT + Pages Read + its own GitHub write/attestation permissions). Each environment's deployment branch policy is selected-branch `main` only; no required reviewers (LD4 — the nightly is unattended, merge review is the human gate, and that limitation is stated rather than papered over). The dispatcher passes no secrets and selects no environment. Probe run `33473559423` is the counterexample that retired the reusable topology: its direct environment-bound job received the temporary secret while its reusable called job received an empty value. Temporary repository-scope copies of the three names remain an owner-controlled, workflow-unreferenced recovery fallback during this incident; agents do not edit or delete them. After one complete supervised environment-based publish succeeds, the owner deletes those copies and a second supervised dispatch proves no fallback (`docs/operations/github-security.md` §3). An absent environment or secret makes every consumer fail **closed**.
- **Non-secret Actions variables inventory (repository variables, read with `vars.` — never `secrets.`).** These are configuration, not credentials, and they are inventoried here because three of them are *switches that arm production behaviour* and one names a path on a machine. Read through `secrets.` a repository variable resolves to the **empty string with no error at all**, which is why every consumer is pinned by a shape test. *(1)* `CLOUDFLARE_ACCOUNT_ID` and *(2)* `CLOUDFLARE_PAGES_PROJECT` — the deploy target (§12.1); *(3)* `POPULUS_PUBLISH_ARMED` — the nightly publish is inert until this equals `true`; *(4)* `POPULUS_RECORD_SIGN_ARMED` — the deployment signer likewise. The top-level signer checks it in a **failing first step**, not a job-level `if`, because a separately dispatched workflow whose only job skips can conclude green; the publish dispatcher watches and propagates that failure before the caller-side assertion runs (§5.5). *(5)* **`POPULUS_INST_DB`** (RUN M2-11) — the absolute path of the accepted institutional source snapshot on the self-hosted runner. **Unset means the flag is omitted entirely and the build is congress-only and byte-identical to a pre-M2-11 build**; the path is provisioned on the repository and never committed, so no machine layout is published in the workflow file. *(6)* **`POPULUS_SELFHOSTED_VALIDATED`** (RUN M2-11) — set to `true` by the owner **only after a supervised `workflow_dispatch` run on the self-hosted machine has been observed end to end**. `schedule` runs additionally require it; `workflow_dispatch` is exempt, because a supervised dispatch is the only way to earn it. Unsetting it is also the first step of runner teardown and the rollback for the whole self-hosted change.
...
*(Retained as the verification contract and phase record: P0–P2 are complete, P3 is live with its stated residuals — see the P3 status paragraph below. The gate and fixture lists remain the definitions the shipped tests implement.)*
...
*End of ARCHITECTURE.md v2.12 — supersedes all earlier versions (v2.1 = `f7985f6`, v2.2 = `5a665ce`, v2.3 = `cfdb972`, v2.4 = `1618fec`, v2.5 = `c85250b`, v2.6 = `a7dee2f`, v2.7 = `3784b25`, v2.8 = `b2ed2ea`, v2.9 = `0f44bec`, v2.10 = `5010258`, v2.11 = `04ac292`). Finding dispositions for all review rounds were recorded in `REVIEW-RESPONSE.md` (Git history).*
