...
| 5 | **Production** upload of provably the same bytes, then live-verified on `publicfilings.org`; any failure triggers the compensating Cloudflare rollback to the captured prior deployment id | prior build restored — **except on the first run**, §3 |
