...
- **R2 (F-2a, render)** No dead holders link: the congress ticker page's "13F institutional
  ...
  `/institutional/tickers/[t]/` exists as a redirect stub to `/tickers/{T}/#institutional`; a
...
| T2 gate congress holders link on `state === "data"`; add `/institutional/tickers/[t]/index.astro` redirect stub; post-build href resolver | R2 | post-build test: every `/institutional/tickers/…` href in `dist/` resolves to a built file; unit test: link absent when state ≠ data | zero dead hrefs in `dist/` |
