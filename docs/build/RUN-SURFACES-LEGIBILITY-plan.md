...
| R10 | Of the **13** production `terminusRow` call sites, the five that sit beside a `compactDisclosure` stating the same count are deleted; the **eight** with no adjacent expand control are kept, named individually in Current State. `syncTerminusFor` loses all callers and is deleted; `terminusRow` itself stays. |
